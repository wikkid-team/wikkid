...
class TestFileStore(object):
    ...
    def test_mimetype(self):
        ...
        self.assertEqual("text/plain", filestore.get_file("simple.txt").mimetype)
        self.assertEqual("text/x-c++src", filestore.get_file("source.cpp").mimetype)
        self.assertEqual("image/jpeg", filestore.get_file("image.jpg").mimetype)
    ...
    def test_updating_file_with_directory_clash(self):
        filestore = self.make_filestore([("first", b"content")])
        user = None
        self.assertRaises(
            FileExists, filestore.update_file, "first/second", b"content", user, None
        )
    ...
    def test_listing_directory_subdir(self):
        ...
        self.assertEqual(["another", "subfile"], sorted(f.base_name for f in listing))
    ...
    def test_last_modified(self):
        ...
        filestore.update_file(
            "new-file.txt", b"some content", "Test Author <test@example.com>", None
        )
        curr = filestore.get_file("new-file.txt")
        end = datetime.utcnow() + timedelta(5)
        # A new line is added to the end too.
        self.assertEqual("Test Author <test@example.com>", curr.last_modified_by)
