...
class TestBzrFileStore(TestCaseWithTransport, ProvidesMixin, TestFileStore):
    ...
    def test_conflicts(self):
        filestore = self.make_filestore([("test.txt", b"one line of content\n")])
        f = filestore.get_file("test.txt")
        base_rev = f.last_modified_in_revision
        # First update succeeds.
        filestore.update_file(
            "test.txt", b"different line\n", "Test Author <test@example.com>", base_rev
        )
        ...
        self.assertEqual(curr.last_modified_in_revision, conflicts.basis_rev)
        self.assertEqual(
            dedent("""\
            <<<<<<<
